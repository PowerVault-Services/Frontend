...
export default function HomeInspection() {
  ...
  const handleEdit = (row: Inspection) => {
    const inspectionData = {
      ...
      date: row.date,
      time: row.time,
      status: row.status, // เก็บ status ไปด้วยเพื่อให้หน้า Step รู้ว่าเป็นโหมดดูอย่างเดียว
    };

    localStorage.setItem("inspection_step1", JSON.stringify(inspectionData));
    localStorage.setItem("jobId", row.id.toString());

    // ถ้าสถานะเป็น COMPLETED ให้ส่งไปหน้า Step 1 เพื่อเริ่มดูแบบ Read-only
  ...
  return (
    <div className="w-full">
      ...
      <div className="flex justify-end mt-[65px]">
        <button onClick={handleDownloadZip} className="flex items-center px-7 py-3 gap-1.5 bg-white shadow-[0px_1px_1px_0px_rgba(0,0,0,0.25)] border-2 border-green-700 rounded-md text-xs text-green-700 font-normal">
          <img src={ZipIcon} alt="" />
      ...
      <div className="pt-[25px]">
        <DataTable<Inspection> columns={columns} data={data} loading={loading} />
      </div>

      <div className="flex items-center justify-between py-6 text-sm text-gray-500">
        <span>{(page - 1) * pageSize + 1} to {Math.min(page * pageSize, totalItems)} of {totalItems} items</span>
